...
|---|---|---|---|---|---|
| Proposition 6, part 1 (output comparison; discrete specialization) / Main text p. 27, item 1; Online Appendix Lemma 2.10, pp. 27 | `discreteOutputStrictlyHigherSpec` -> `discreteOutputStrictlyHigher` | discrete arithmetic target | partial: source audit pending | no added assumptions | `by intro q hlo hhi; linarith` |
